...
const DATA_SOURCES = [
  {
    ...
    color: "border-hong/30 bg-hong/5",
    badge: "핵심 데이터",
    badgeColor: "bg-hong text-white",
  },
  {
    name: "전통복식 한복 이미지",
    count: "10,163 건",
    ...
    color: "border-cheong/30 bg-cheong/5",
    badge: "생성 기반",
    badgeColor: "bg-cheong text-white",
  },
  {
    name: "한국민족문화대백과사전 QA",
    count: "표제어 기반",
    ...
    color: "border-geum/30 bg-geum/5",
    badge: "고증 RAG",
    badgeColor: "bg-geum text-white",
  },
  {
    name: "전통문양조회 API",
    count: "API 실시간",
    ...
    color: "border-muk/20 bg-baek",
    badge: "폴백 API",
    badgeColor: "bg-muk text-white",
  },
];
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      <section className="mb-12">
        <h2 className="mb-5 text-lg font-bold text-muk">활용 데이터 목록</h2>
        <div className="space-y-4">
          {DATA_SOURCES.map((ds) => (
            <div
              key={ds.name}
              className={`rounded-2xl border p-5 shadow-card ${ds.color}`}
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-sm font-bold text-muk">{ds.name}</h3>
                    <span
                      className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${ds.badgeColor}`}
                    >
                      {ds.badge}
                    </span>
                  </div>
                  <p className="text-xs text-muk/55 mb-2">{ds.role}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {ds.fields.map((f) => (
                      <span
                        key={f}
                        className="rounded-full border border-muk/10 bg-white/60 px-2 py-0.5 text-[11px] text-muk/60"
                      >
                        {f}
                      </span>
                    ))}
                ...
                <div className="text-right shrink-0">
                  <p className="text-xl font-extrabold text-geum">{ds.count}</p>
      ...
      <section className="mb-12">
        <h2 className="mb-5 text-lg font-bold text-muk">
        ...
        <div className="rounded-2xl border border-geum/15 bg-white p-6 shadow-card">
          ...
          <div className="overflow-x-auto">
            ...
            >
              {/* 배경 */}
              <rect width="720" height="260" fill="#f8f4ef" rx="16" />

              {/* 노드들 */}
              {/* 1. 사연 입력 */}
              <rect x="20" y="100" width="110" height="60" rx="10" fill="#1a4a7a" />
              ...
              {/* 5. 결과 출력 */}
              <rect x="620" y="100" width="85" height="60" rx="10" fill="#1a1410" />
              <text x="662" y="127" textAnchor="middle" fill="white" fontSize="11" fontWeight="bold">결과 출력</text>
              <text x="662" y="143" textAnchor="middle" fill="#aaa" fontSize="10">+ 출처 각주</text>
              ...
              />
              <text x="446" y="228" textAnchor="middle" fill="#c0392b" fontSize="10" fontWeight="bold">
                &lt;70 → 재생성
              </text>

              {/* 민속대백과 RAG 연결 */}
              <rect x="310" y="20" width="120" height="44" rx="8" fill="#e8f0f8" stroke="#1a4a7a" strokeWidth="1.5" />
          ...
          <p className="mt-4 text-xs text-center text-muk/45">
      ...
      {/* ── 성능 검증 지표 ── */}
      <section className="mb-12">
        <h2 className="mb-5 text-lg font-bold text-muk">성능 검증 지표</h2>
          ...
          {METRICS.map((m) => (
            <div
              ...
              className={`rounded-2xl border p-5 shadow-card ${
                m.status === "active"
                  ? "border-geum/25 bg-white"
                  : "border-muk/10 bg-baek"
              }`}
            >
              <div className="flex items-start justify-between gap-2">
                <p className="text-sm font-bold text-muk">{m.label}</p>
                <span
              ...
              <p className="mt-2 text-xl font-extrabold text-geum">{m.value}</p>
        ...
        {/* 혼동행렬 placeholder */}
        <div className="mt-4 rounded-2xl border border-muk/10 bg-baek p-6 text-center">
          <p className="text-xs font-semibold text-muk/40 uppercase tracking-widest mb-3">
      ...
      <section className="mb-10">
        <h2 className="mb-5 text-lg font-bold text-muk">생성물 권리구조</h2>
        <div className="rounded-2xl border border-geum/15 bg-white p-6 shadow-card">
            ...
            {[
              {
                ...
                color: "bg-cheong/8 border-cheong/20",
              ...
              },
              {
                ...
                color: "bg-geum/8 border-geum/20",
                textColor: "text-geum",
              },
              {
                title: "로열티 한정",
                ...
                color: "bg-hong/8 border-hong/20",
                textColor: "text-hong",
              },
            ].map((r) => (
              <div key={r.title} className={`rounded-xl border p-4 ${r.color}`}>
                <p className={`text-sm font-bold ${r.textColor}`}>{r.title}</p>
                <p className="mt-1.5 text-xs font-semibold text-muk">{r.desc}</p>
                <p className="mt-1 text-[11px] text-muk/50">{r.note}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="text-center">
        <Link
          href="/studio"
          className="inline-flex items-center gap-2 rounded-xl bg-hong px-7 py-3.5 text-sm font-bold text-white shadow-lg transition hover:bg-hong-light"
        >
          스튜디오에서 직접 검증하기 →
        </Link>
      </div>
